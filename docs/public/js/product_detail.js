$(document).ready(function () {
  ...
  $("#btn-consult").click(function () {
    var formStatus = $("#form-contact-modal")
      .validate({
        rules: {
          name: "required",
          phone: "required",
  ...
  $(".main-slider").each(function (index) {
    ...
    var slideCount = $mainSlider.find(".swiper-slide-main").length;

    // Nếu chỉ có 1 ảnh -> không cần slick
    if (slideCount <= 1) {
      $mainSlider.slick({
        slidesToShow: 1,
        infinite: false,
      ...
      // Ẩn luôn slide nếu chỉ có 1 ảnh
      $thumbSlider.css("display", "none");

      return; // bỏ qua init thumbs
    }
    // Nếu từ 2 -> 5 ảnh
    if (slideCount <= 5) {
      $mainSlider.slick({
      ...
      // Chặn slick-track của thumbnail di chuyển bằng CSS
      $thumbSlider.addClass("static-thumbs");
    }
