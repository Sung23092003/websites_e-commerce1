...
const addThemeStyles = (themes, api) => {
    ...
    for (const { name, selectors: _selectors, extend, mediaQuery } of themes) {
        const selectors = _selectors !== null && _selectors !== void 0 ? _selectors : (name === optionsUtils_1.defaultThemeName
            ? [':root']
            : mediaQuery
                ? []
                : [`.${e(name)}`]);
...
const multiThemePlugin = plugin_1.default.withOptions((options = defaultOptions) => api => {
    ...
    const extension = (0, themeUtils_1.resolveThemeExtensionsAsTailwindExtension)((0, optionsUtils_1.getThemesFromOptions)(options));
    return {
        theme: {
            extend: extension
        }
